...
<!--  login part -->
        <head>
            <title>Login - Fitness Membership System</title>
        ...
        <body>
            <div class="login-card">
                ...
                <div class="form-footer">
                    ...
                    <p>Are you an admin? <a href="/users/admin/login">Admin Login</a></p>
                </div>
            </div>
            <script>
                // If redirected to profile page after login, store user id in localStorage
                document.addEventListener('DOMContentLoaded', function() {
                    // Try to extract user id from the URL if on /users/profile/{userId}
                    var match = window.location.pathname.match(/\/users\/profile\/(.+)$/);
                    if (match && match[1]) {
                        localStorage.setItem('userId', match[1]);
                        console.log('Stored user ID:', match[1]);
                    }
                });
            </script>
        </body>
